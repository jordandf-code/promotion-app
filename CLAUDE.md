...
Layers 0–3 complete (2I Calendar deferred). All 22 roadmap features shipped. Layer 4 scoped (4A Viewer Access, 4B View Others) but deferred. Issue triage (2026-04-09) produced prioritized bug/fix backlog — see `docs/PLAN.md` "Issue triage" section. Waves 1-4 complete: all P0-P3 backlog items resolved (20 items). Security Scan (#81) complete: S1-S4 (Helmet.js, CI/CodeQL, Playwright security tests, weekly scans). Competency Radar v2 (#82) complete: C1-C3 (BARS wizard, evidence linking, multi-rater + Johari Window). UI overhaul complete: design tokens (spacing/typography/shadow/color), grouped sidebar nav (7 groups, collapsible), button consolidation (3 variants), progressive disclosure (Dashboard, Competencies), Cmd+K command palette, collapsible sidebar rail, empty states, design system docs. Nav groups defined in `frontend/src/navGroups.js`. New tabs go to `testing` group (superuser-only). Nav trimmed to 14 core tabs (2026-04-18): 10 non-core tabs (AI-heavy, visual-only, deferred) moved to superuser-only `testing` group with route guards. Issue sweep (2026-04-12): closed 18 of 26 issues. P4 batch (2026-04-12): #63 DIY JSON import, #92 AI action item extraction, #101 per-domain import/export (goals, actions, learning, eminence), #85 already done (closed). Scorecard polish batch (2026-05-14): #111 decimal inputs, % progress on Overview tiles, sortable columns on Opps/Projects tables. #104 FirmConfigSection moved from SuperAdmin to Admin GenAI tab. Issue batch (2026-07-24, parallel A–D): #118 new terminal `no_pursue` opportunity status (excluded from win-rate/pipeline/forecast/AI); #112 Team signings/revenue tiles on Opportunities tab driven by manual team target+actual on Targets tab; #115 "Generate project" button creates a linked forecast project from an opportunity; #101 per-tab import/export extended to Targets + Projects; #119 goals carry a second independent `isPerformanceGoal` ("SF performance goal") boolean alongside `isGate` (form checkbox, card badge, filter, CSV, AI context); #117 attach documents to wins via the vault (`vaultFiles.js` shared helper, win card attach/list/remove, docs survive win deletion); #107 pre-send AI cost estimate (`backend/ai/pricing.js` shared pricing, `POST /api/ai/estimate`, `callAI` wrapper + `AICostModal` gate on 18 triggers); #106 real drag-and-drop tab order in Admin (shared `useDragReorder` hook, ↑/↓ buttons kept as fallback). #90/#61 closed as ops tasks (Resend domain verification + Render keep-alive — not code). Open: #24/#19 viewer access (Layer 4, deferred), 2I calendar integration (deferred). Follow-up batch (2026-07-25): CI dependency-audit gate scoped to production deps (`scripts/audit-gate.mjs`, `npm audit --omit=dev` + documented allowlist) — backend has 0 prod highs, frontend patched vite/postcss/undici, react-router held on v7 with a documented SPA exception (react-router v8 migration deferred as its own task); LinkedIn import (2E) completed — added `POST /api/ai/parse-linkedin` (+ `PARSE_LINKEDIN_PROMPT`) and wired the previously-orphaned `LinkedInImportModal` into the People page. Migrations required: `migration_layer0e.sql`, `migration_1c_feedback.sql`, `migration_1g_ai_log.sql`, `migration_c1_question_bank.sql`.
